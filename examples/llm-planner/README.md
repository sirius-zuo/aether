...
Aether's LLM-planning loop, end to end, with **real local-LLM agents** — in a single `cargo run`.

A **planner** agent decomposes a decision question into a workflow DAG; Aether dispatches several **distinct worker agents** in parallel; a **synthesizer** (terminal node) merges their outputs; and `main()` — the caller — receives the final synthesis back from `Orchestrator::submit`. `aether-core` itself stays LLM-free: the "brain" is just another agent that speaks the Envelope protocol.

## Architecture

```
main() ──goal──► Orchestrator::submit
   ...
   analyze_pros   analyze_cons   assess_cost      ← 3 distinct LLM worker agents
        └─────────────┼─────────────┘   (fan-in: array of the 3 payloads)
                 ...
                 synthesize (terminal LLM agent) ──► balanced recommendation
                      │
                      ▼
        Outcome::Success(result) ──► printed by main()
```

The binary spins up six in-process axum agents, each wrapping a shared AgentVerse `LlmRunner`:
...
| cost    | 9105 | `assess_cost`    | Estimate effort / cost / migration risk           |
...
1. `main()` builds a goal `{"goal": "<question>"}` and calls `Orchestrator::submit`.
...
5. The analysts' outputs fan in (as a JSON array) to the **synthesizer**, the single terminal node.
6. The synthesizer's output is returned as `Outcome::Success` and printed under `=== Synthesis ===`.
...
cargo run -p example-llm-planner -- "Should we migrate our API from REST to gRPC?"
```

Pass your own question as the first argument; omit it to use the default.
...
Goal: Should we migrate our API from REST to gRPC?
 ...
 INFO llm-planner: planner produced DAG dag={"nodes":[...]}
 INFO llm-planner: invoke received agent=context node=context
 INFO llm-planner: invoke received agent=pros node=pros
 INFO llm-planner: invoke received agent=cons node=cons
 INFO llm-planner: invoke received agent=cost node=cost
 INFO llm-planner: invoke received agent=synth node=synth
...
- **Observability is console-based** (tracing logs on stderr + the printed synthesis). The live Aether dashboard is out of scope: `Orchestrator::submit` runs on its own internal `Supervisor`, which a dashboard `AppState` does not observe without a change to `aether-core`.
