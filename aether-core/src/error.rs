...
mod tests {
    ...
    fn transport_error_display() {
        let e = AetherError::TransportError { node: "intake".into(), message: "broken pipe".into() };
        assert_eq!(e.to_string(), "transport error on 'intake': broken pipe");
    }

    #[test]
    fn registry_error_display() {
        let e = AetherError::RegistryError { message: "unknown node 'foo'".into() };
        assert_eq!(e.to_string(), "registry error: unknown node 'foo'");
    }

    #[test]
    fn workflow_error_display() {
        let e = AetherError::WorkflowError { message: "cycle detected".into() };
    ...
    fn outcome_failed_roundtrip() {
        let o = Outcome::Failed { node: "writer".into(), error: "out of memory".into() };
        let json = serde_json::to_string(&o).unwrap();
        let back: Outcome = serde_json::from_str(&json).unwrap();
        match back {
    ...
    fn outcome_timeout_roundtrip() {
        let o = Outcome::Timeout { node: "researcher".into() };
        let json = serde_json::to_string(&o).unwrap();
        let back: Outcome = serde_json::from_str(&json).unwrap();
        match back {
